...
namespace Practica
{
    public partial class EliminarMascota : Form
    {
        ...
        {
            DialogResult result = MessageBox.Show("Estás a punto de eliminar una mascota, ¿estás seguro?", "Confirmación", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
            ...
            if (result == DialogResult.Yes)
            {
                try
                {
                    ...
                    using (SqlConnection conexion = BaseDatos.ObtenerConexion())
                    {
                        ...
                        string query = @"DELETE FROM Mascotas WHERE ID LIKE @filtro";
                ...
                {
                    MessageBox.Show("No se puede eliminar esta mascota ya que tiene citas asociadas");
                }
            }
        }
